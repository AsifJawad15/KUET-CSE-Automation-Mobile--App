...
class _StudentGeoAttendanceScreenState extends State<StudentGeoAttendanceScreen>
  ...
  Future<void> _submitAttendance(String roomId) async {
    ...
    try {
      room = _openRooms.firstWhere((r) => r['id'] == roomId);
    } catch (_) {}

    String? enteredCode;
    if (room != null) {
      final codeMap = room['geo_attendance_codes'];
      final dbCode = codeMap is Map ? codeMap['code'] as String? : null;
      if (dbCode != null && dbCode.isNotEmpty) {
        enteredCode = await _showVerificationCodeDialog(dbCode, isDarkMode);
        if (enteredCode == null) {
          // User cancelled code verification
          return;
        }
      }
    }
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(
                    'Enter the 6-digit temporary code generated by the teacher.',
                    style: TextStyle(
                      fontSize: 13,
